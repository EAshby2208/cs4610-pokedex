        ...
        <section className="w-full max-w-2xl">
          ...
          <ul className="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {/* {locations.results.map((loc) => (
              <li key={loc.name} className="border-2 border-green-300 dark:border-green-500 bg-green-100 dark:bg-green-800 rounded-lg p-4 flex flex-col items-center hover:shadow-lg hover:bg-green-200 dark:hover:bg-green-700 transition-all">
                <Link href={`/locations/${loc.name}`} className="flex flex-col items-center text-black dark:text-black">
